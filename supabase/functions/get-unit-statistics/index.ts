...
serve(async (req) => {
  ...
  try {
    const { sheets_ids, unidad } = await req.json();

    if (!sheets_ids?.calificaciones_spreadsheet_id || !unidad) {
      throw new Error("Faltan parámetros: 'sheets_ids.calificaciones_spreadsheet_id' o 'unidad'.");
    }
    ...
    const payload = {
      action: "get_final_unit_grades",
      spreadsheetId: sheets_ids.calificaciones_spreadsheet_id,
      unidad: unidad,
    };
