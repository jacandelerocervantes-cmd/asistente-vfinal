...
serve(async (req: Request) => {
  ...
  try {
    ...
    try {
      ...
      const errorMessage = e instanceof Error ? e.message : "Error desconocido durante el procesamiento del trabajo.";
      await supabaseAdmin.from('cola_de_trabajos').update({ estado: 'fallido', ultimo_error: errorMessage, intentos: (trabajo.intentos || 0) + 1 }).eq('id', trabajo.id);
      throw e;
    }
